                                ...
                                @foreach ($categories as $value)
                                    ...
                                    <div class="panel panel-default">
                                            <div class="panel-heading" role="tab" id="headingOne">
                                                ...
                                                <a data-toggle="collapse" data-parent="#accordion" href="#menu{{$value->id}}"
                                                        aria-expanded="true" aria-controls="collapseOne">{{$value->name}}
                                                    </a>
                                                </h4>
                                            </div>

                                            <div id="menu{{$value->id}}" class="panel-collapse collapse" role="tabpanel"
                                                aria-labelledby="headingOne">
                                                <div class="panel-body">
                                                    <ul>
                                                    @foreach ($categories as $row)
                                                    @if ($row->parent==$value->id)
                                                        <li><a href="/product/shop/{{$row->id}}">{{$row->name}}</a></li>
                                                    @endif
                                                    @endforeach
                                                    </ul>
                                                </div>
